...
def pronostico(carrera_id):
    ...
    # 2️⃣ Obtener pilotos
    cur.execute("SELECT nombre FROM pilotos ORDER BY nombre ASC")
    pilotos = [p["nombre"] for p in cur.fetchall()]

    print("DEBUG pilotos:", pilotos)  # 👈 CLAVE

    # 3️⃣ Buscar pronóstico existente
    cur.execute("""
        SELECT *
        FROM pronosticos
    ...
    pronostico_existente = cur.fetchone()

    # 4️⃣ Guardar / actualizar
    if request.method == "POST" and carrera["pronostico_abierto"]:
        pole = request.form.get("pole")
        sprint = request.form.get("sprint_ganador")
        ...
        if pronostico_existente:
            cur.execute("""
                ...
                WHERE id=?
            """, (pole, sprint, p1, p2, p3, pronostico_existente["id"]))
        else:
            cur.execute("""
                INSERT INTO pronosticos
                (user_id, carrera_id, pole, sprint_ganador, p1, p2, p3)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (session["user_id"], carrera_id, pole, sprint, p1, p2, p3))

        conn.commit()
        conn.close()
        return redirect(f"/fecha/{carrera_id}")
    ...
    return render_template(
        ...
        carrera=carrera,
        pilotos=pilotos,
        pronostico=pronostico_existente
    )
